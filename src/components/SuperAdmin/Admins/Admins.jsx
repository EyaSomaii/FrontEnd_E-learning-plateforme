...
import Liste_Admins from "../../Containers/Liste_Admins";
...
export default function Admins() {
  ...
  const filterContent = async (user, searchTerm) => {
    const result = user.filter(
      (us) =>
        us.nom.toLowerCase().includes(searchTerm) ||
        us.prenom.toLowerCase().includes(searchTerm) ||
        us.email.toLowerCase().includes(searchTerm)
    );
    setUser(result);
  ...
  const handleTextSearch = async (e) => {
    ...
    axios.get("http://localhost:3001/User/Administrateurs").then((res) => {
  ...
  const SuppUser = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Swal.fire("Suppression fait avec succès", "", "success");
        axios
          .delete(`http://localhost:3001/User/SupprimerUti/${id}`)
          .then((res) => {
  ...
  const getUserData = async () => {
    try {
      const data = await axios.get(
        "http://localhost:3001/User/Administrateurs"
      );
          ...
          <div className="list">
              ...
              >
                <Badge colorScheme="purple" fontSize="1em" borderRadius="full">
                  <p style={{ textAlign: "center" }}>
                    Liste des administrateurs ( {user.length} )
                  </p>
                </Badge>
              </Stack>
            </ChakraProvider>
            ...
            >
              <input
                ...
                type="search"
                placeholder="Recherche administrateur"
                name="searchTerm"
